...
public class BeeDatabaseTab implements IDatabaseTab<IBee> {
	...
	@Override
	public void createElements(IGuiElementGenetic container, IBee bee, ItemStack itemStack) {
		...
		if (type == EnumBeeType.PRINCESS || type == EnumBeeType.QUEEN) {
			...
			container.text(displayText, GuiElementAlignment.CENTER, ColourProperties.INSTANCE.get("gui.beealyzer.binomial"));
		}
	}

	protected void addRow(IGuiElementHelper elementHelper, String leftText, String rightText, boolean dominant){
		IGuiElementContainer panel = elementHelper.factory().createPanel(0, 0, elementHelper.getParent().getWidth(), 12);
		panel.text(leftText + ": ", GuiElementAlignment.CENTER_LEFT);
		panel.text(rightText, GuiElementAlignment.CENTER_RIGHT, elementHelper.factory().getColorCoding(dominant));
		elementHelper.add(panel);
	}
