...
import forestry.core.gui.elements.ElementHelper;
...
import forestry.core.gui.elements.layouts.PanelElementLayout;
import forestry.core.gui.elements.layouts.VerticalElementLayout;
...
public class TextPageParser implements IBookPageFactory {
	...
	private static class PageBuilder {
		...
		@Nullable
		private ElementHelper currentPage;
		@Nullable
		...
		public void add(Image image) {
			...
			page.add(page.centerElementX(new DrawableElement(0, 0, image.width, image.height, image.drawable)));
		}
		...
		public void add(IRecipe recipe) {
			...
			IGuiElementHelper page = page(firstRecipe ? 72 : 60);
			PanelElementLayout panel = new PanelElementLayout(98, firstRecipe ? 72 : 60);
			int gridStartY = 0;
			if(firstRecipe) {
		...
		private void addCarpenter(ICarpenterRecipe recipe){
			...
			ElementContainer panel = GuiElementFactory.INSTANCE.createPanel(0, 0, 108, firstRecipe ? 72 : 60);
			int gridStartY = 0;
			if(firstRecipe){
				panel.text(TextFormatting.DARK_GRAY + "Carpenter", GuiElementAlignment.CENTER);
				gridStartY = 12;
			}
		...
		public void add(String line, boolean center) {
			add(new TextElement(-1, 9, line, GuiElementAlignment.LEFT, 0, true), center);
		}
		...
		private void add(IGuiElement element, boolean center){
			ElementHelper helper = page(element.getHeight());
			if(center){
				helper.centerElementX(element);
			}
			helper.add(element);
		}

		private ElementHelper page(int height) {
			return page(false, height);
		}

		private ElementHelper page(boolean forced, int elementHeight) {
			if (currentPage == null) {
		...
		private ElementHelper createNewPage() {
			IGuiElementLayout page = new VerticalElementLayout(108);
