...
public class TreeDatabaseTab implements IDatabaseTab<ITree> {
	...
	@Override
	public void createElements(IGuiElementGenetic container, ITree tree, ItemStack itemStack) {
		...
		container.text(Translator.translateToLocal("for.gui.database.tab." + (active ? "active" : "inactive") + "_species.name"), GuiElementAlignment.CENTER, 0xcfb53b);
		...
		container.text(TextFormatting.UNDERLINE + Translator.translateToLocal("for.gui.supports"), GuiElementAlignment.CENTER);
		List<IFruitFamily> families = new ArrayList<>(tree.getGenome().getPrimary().getSuitableFruit());

		for (IFruitFamily fruitFamily : families) {
			container.text(fruitFamily.getName(), GuiElementAlignment.CENTER, speciesColor);
		}
		...
		container.text(TextFormatting.UNDERLINE + Translator.translateToLocal("for.gui.fruits"), GuiElementAlignment.CENTER);
		String strike = "";
		...
		container.text(strike + fruit.getProvider().getDescription(), GuiElementAlignment.CENTER, colorCoding);

		IFruitFamily family = fruit.getProvider().getFamily();

		if (family != null && !family.getUID().equals(EnumFruitFamily.NONE.getUID())) {
			container.text(TextFormatting.UNDERLINE + Translator.translateToLocal("for.gui.family"), GuiElementAlignment.CENTER);
			container.text(family.getName(), GuiElementAlignment.CENTER, colorCoding);
		}
