...
import forestry.core.gui.elements.layouts.HorizontalElementLayout;
...
public class GuiElementFactory implements IGuiElementFactory {
	...
	public AbstractElementLayout createVertical(int xPos, int yPos, int width) {
		return new VerticalElementLayout(xPos, yPos, width);
	}

	@Override
	public AbstractElementLayout createHorizontal(int xPos, int yPos, int height) {
		return new HorizontalElementLayout(xPos, yPos, height);
	}

	@Override
	public ElementContainer createPanel(int xPos, int yPos, int width, int height) {
		return new PanelElementLayout(xPos, yPos, width, height);
	}
	...
	public IGuiElementLayout createToleranceInfo(IAlleleTolerance toleranceAllele) {
		...
		IGuiElementLayout layout = createHorizontal(0, 0, 0).setDistance(2);
	...
	public IGuiElementContainer createMutationResultant(int x, int y, int width, int height, IMutation mutation, IBreedingTracker breedingTracker) {
		if (breedingTracker.isDiscovered(mutation)) {
			IGuiElementContainer element = new PanelElementLayout(x, y, width, height);
	...
	public IGuiElementContainer createMutation(int x, int y, int width, int height, IMutation mutation, IAllele species, IBreedingTracker breedingTracker) {
		if (breedingTracker.isDiscovered(mutation)) {
			PanelElementLayout element = new PanelElementLayout(x, y, width, height);
	...
	private static void createProbabilityArrow(PanelElementLayout element, IMutation combination, int x, int y, IBreedingTracker breedingTracker) {
