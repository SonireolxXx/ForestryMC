...
public class GeneticAnalyzerTabs extends VerticalElementLayout {
	...
	private class Tab extends GuiElement{
		...
		public void draw(int startX, int startY, int mouseX, int mouseY) {
			...
			background.draw(startX + getX(), startY + getY());
			if (!displayStack.isEmpty()) {
				RenderHelper.enableGUIStandardItemLighting();
				GlStateManager.enableRescaleNormal();
				GuiUtil.drawItemStack(Minecraft.getMinecraft().fontRenderer, displayStack, getX() + startX + 9, getY() + startY + 5);
