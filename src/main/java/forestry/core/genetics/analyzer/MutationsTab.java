...
public class MutationsTab extends DatabaseTab {
	...
	@Override
	public void createElements(IGuiElementGenetic container, IIndividual individual, ItemStack itemStack) {
		...
		IGuiElementLayoutHelper groupHelper = container.layoutHelper((x, y) -> GuiElementFactory.INSTANCE.createHorizontal(x + 1, y, 16), 100, 0);
		Collection<? extends IMutation> mutations = getValidMutations(speciesRoot.getCombinations(species));
		if(!mutations.isEmpty()) {
			container.text(TextFormatting.UNDERLINE + Translator.translateToLocal("for.gui.database.mutations.further"), GuiElementAlignment.CENTER);
		...
		container.text(TextFormatting.UNDERLINE + Translator.translateToLocal("for.gui.database.mutations.resultant"), GuiElementAlignment.CENTER);
