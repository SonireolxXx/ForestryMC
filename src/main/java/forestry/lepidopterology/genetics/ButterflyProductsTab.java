...
public class ButterflyProductsTab implements IDatabaseTab<IButterfly> {
	...
	@Override
	public void createElements(IGuiElementGenetic container, IButterfly individual, ItemStack itemStack) {
		IGuiElementLayoutHelper groupHelper = container.layoutHelper((x, y) -> GuiElementFactory.INSTANCE.createHorizontal(x + 4, y, 18).setDistance(2), 90, 0);
		Collection<ItemStack> butterflyLoot = individual.getGenome().getPrimary().getButterflyLoot().keySet();
		if(!butterflyLoot.isEmpty()) {
			container.text(Translator.translateToLocal("for.gui.loot.butterfly"), GuiElementAlignment.CENTER);
		...
		if(!caterpillarLoot.isEmpty()){
			container.text(Translator.translateToLocal("for.gui.loot.caterpillar"), GuiElementAlignment.CENTER);
		...
		if(!cocoonLoot.isEmpty()){
			container.text(Translator.translateToLocal("for.gui.loot.cocoon"), GuiElementAlignment.CENTER);
